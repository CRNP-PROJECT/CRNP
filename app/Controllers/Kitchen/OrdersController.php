...
/**
 * OrdersController — Kitchen Display: active orders + status workflow.
 *
...
class OrdersController {
              ...
              <tbody>
                <?php foreach ($orders as $id => $o): ?>
                  <?php
                    ...
                    $elapsed = self::elapsed((string)($o['created_at'] ?? ''));
                  ?>
                  <tr>
                    <td><span class="kbd">#<?= e(self::shortId((string)$id)) ?></span></td>
                    <td><?= e(self::customerName($o)) ?></td>
                    <td><?= self::itemsHtml($o['items'] ?? []) ?></td>
                      ...
                      <?php elseif ($s === 'ready'): ?>
                        <form method="post" action="/kitchen/" class="row">
                          ...
                          <button type="submit" class="btn btn--ghost btn--sm"
                                  data-confirm="Mark order #<?= e(self::shortId((string)$id)) ?> as done?">Mark done</button>
                        </form>
                      <?php else: ?>
    ...
    private static function handlePost(): void {
        ...
        try {
            $order->update($patch);
            flash('Order #' . self::shortId($orderId) . ' moved to ' . ucfirst($newStatus) . '.', 'ok');
        } catch (Throwable $ex) {
    ...
    private static function itemsHtml($items): string {
    ...
    private static function elapsed(?string $ts): string {
