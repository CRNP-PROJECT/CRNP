...
/**
 * Mailer — email delivery through Gmail SMTP (STARTTLS, port 587).
 * Sends OTPs and HTML receipt emails via PHPMailer.
 */
class Mailer {
    ...
    public static function sendOtp(string $email, string $otp): bool {
        ...
        try {
            ...
            $mail->Body    = self::otpEmailHtml($otp);
    ...
    public static function sendOrderReceipt(string $email, array $order): bool {
        $brand   = BRAND_NAME;
...
HTML;
        ...
        return self::send($email, $subject, $html, $alt);
    }

    public static function sendBookingReceipt(string $email, array $booking): bool {
        $brand   = BRAND_NAME;
        $tagline = BRAND_TAGLINE;
...
HTML;
    ...
    private static function otpEmailHtml(string $otp): string {
        $brand   = BRAND_NAME;
        $tagline = BRAND_TAGLINE;
        return <<<HTML
<!doctype html><html><body style="margin:0;background:#f6f2ea;font-family:Georgia,'Times New Roman',serif;">
